...
def load_model():
    return YOLO('yolov8n.pt')  # Load the pre-trained YOLOv8 nano model
...
def process_video(model, video_path):
    ...
    vehicle_trajectories = {}  # Dictionary to store vehicle trajectories
    ...
    while cap.isOpened():  # Loop through video frames
        ...
        for box in results[0].boxes.data:  # Loop through detected objects
            ...
            dimensions = estimate_dimensions((x1, y1, x2, y2), height, width)
            ...
            if cls not in vehicle_trajectories:
                vehicle_trajectories[cls] = []
            vehicle_trajectories[cls].append(center)
            if len(vehicle_trajectories[cls]) > 10:  # Keep only last 10 positions
                vehicle_trajectories[cls].pop(0)
            
            if len(vehicle_trajectories[cls]) >= 2:
                next_point = predict_trajectory(vehicle_trajectories[cls][:-1], center)
...
def main(input_path):
    ...
    # Note: counting is now done frame by frame in process_video
